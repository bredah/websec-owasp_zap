...
def add_proxy_scripts(zap):
    script_dir = os.path.join(os.path.dirname(
        os.path.realpath(__file__)), "proxy_scripts")
    for script in os.listdir(script_dir):
        filename = os.path.join(script_dir, script)
        # Note: scriptDescription parameter is required - API bug
        zap.script.load(script, "proxy",
                        "ECMAScript : Oracle Nashorn", filename, "")
...
def main(argv):
    ...
    config_dict = shared.read_config_file(
        config_read) if config_read is not None else {}
